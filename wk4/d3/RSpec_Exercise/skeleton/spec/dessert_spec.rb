...
describe Dessert do
  let(:chef) { double("chef", name: "Marco") }
  ...
  describe "#initialize" do
    ...
    it "raises an argument error when given a non-integer quantity" do
      expect { Dessert.new("cake", "liters", chef) }.to raise_error(ArgumentError)
    end
  end
  ...
  it "shuffles the ingredient array" do
    ingredients = ["chocolate", "flour", "egg", "sugar", "butter", "water"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name" do
      allow(chef).to receive(:titleize).and_return("Chef Marco the Great Baker")
      expect(brownie.serve).to eq("Chef Marco the Great Baker has made 10 brownies!")
    end
  end
